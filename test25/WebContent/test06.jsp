...
<%
	...
	out.println("<h3>session(세션)은 로그아웃 전 또는 브라우저를 종료하기 전까지 그정보가 유지된 객체</h3>");
	...
	out.println("<h3>application(앱)은 브라우저 종료 전까지 그정보가 유지된다.</h3>");
	...
	out.println("<h2>page(페이지) :현재 페이지 정보를 저장하는객체</h2>");
	out.println("<h3>page(페이지)는 그 정보가 현재 페이지 내에서만 정보가 유지된다.</h3>");
	out.println("<h3>전송된 값은 그정보가 목적지까지만 유지되며, 다른 페이지에서는 해당 정보를 </h3>");
	out.println("<p>웹 환경에서의 Scope : 그 정보가 어디까지 또는 언제까지 유지</p>");
	out.println("page < trans < session < application");
	
	//session.setAttribute("sid","lee"); -> 모든 8대 객체에 저장시는 setAttribute("필드명","값")를 사용//sid변수에 저장되는 lee
	//session.getAttribute("sid"); -> 모든 8대 객체에서 값을 가져올 경우는 getAttribute("필드명") 단, request 객체의 값을 가져올경우는 getParameter("파라미터명")
%>
<div>
