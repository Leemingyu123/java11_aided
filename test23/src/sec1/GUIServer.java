...
class ManagerLogin extends JFrame implements ActionListener, KeyListener{	//GUI 관리자 창
	...
	JLabel port_warnning = new JLabel("(단, 포트번호는 0~65536 이어야함 )");
	...
	public ManagerLogin() {	
		...
		setVisible(true);
		setResizable(false);
		port_btn.addActionListener(this);	//누르면동작하는버튼
		port_num.addKeyListener(this);
		port_log.add(port_label);
		port_log.add(port_warnning);
	...
	@Override
	public void keyTyped(KeyEvent e) {	//키를 눌렀다가 떼었을 때
		
	}

	@Override
	public void keyPressed(KeyEvent e) {	//키보드 입력(누를때)
		try {
	...
	@Override
	public void keyReleased(KeyEvent e) {	//누르고 있떤 키보드의 키를 떼었을 때 
		
	}

	@Override
	public void actionPerformed(ActionEvent e) {	//마우스클릭
		//접속~! 버튼을 누르면
		try {
				int port = Integer.parseInt(port_num.getText().trim());
...
class ChatServerGUI extends JFrame implements ActionListener, KeyListener{
	...
	JTextField chat = new JTextField(45);	//한줄짜리
	JButton enter = new JButton("전송");
	TextArea serverChatList = new TextArea(30, 50);	//여러줄짜리
	...
	public void appendMessage(String message) {	//메시지 추가동작
