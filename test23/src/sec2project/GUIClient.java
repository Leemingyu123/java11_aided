...
class LoginGUI extends JFrame implements ActionListener {
	...
	public LoginGUI() {
		...
		setResizable(false);
		setVisible(true);
		IPAddress_Label.setFont(IPAddress_Label.getFont().deriveFont(13.0f));
		NickName_Label.setFont(NickName_Label.getFont().deriveFont(13.0f));
		Port_Label.setFont(Port_Label.getFont().deriveFont(13.0f));
		Login_GUI_Button.setFont(Login_GUI_Button.getFont().deriveFont(15.0f));
		Login_GUI_Button.setBackground(Color.lightGray);
		Login_GUI_Button.setPreferredSize(new Dimension(80, 30));
		Login_GUI_Button.addActionListener(this);
		Login_GUIPanel.setBackground(Color.orange);
...
class ChatClientGUI extends JFrame implements ActionListener, KeyListener {
	...
	public ChatClientGUI(String NickName, String IPAddress, int Port) {
		...
		ClientGUIPanel.setBackground(Color.orange);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == Enter && Message.length() > 0) {
			CB.Transmit("["+formatedNow+"]"+NickName + " : " + Message + "\n");
			Chat.setText(null);
		}
	}
