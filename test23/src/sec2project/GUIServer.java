...
class ManagerLogin extends JFrame implements ActionListener, KeyListener {	//GUI 관리자 창 
	...
	public ManagerLogin() {
		...
		setVisible(true);
		setResizable(false);
		port_label.setFont(port_label.getFont().deriveFont(15.0f));
		port_label.setForeground(Color.white);
		port_warn.setForeground(Color.white);
		port_btn.setPreferredSize(new Dimension(80, 30));
		port_btn.addActionListener(this);
		port_log.setBackground(Color.darkGray);
		port_btn.setBackground(Color.gray);
		port_btn.setForeground(Color.white);
		port_btn.setFont(port_btn.getFont().deriveFont(15.0f));
...
class ChatServerGUI extends JFrame implements ActionListener, KeyListener {
	...
	public ChatServerGUI(int port) {
		...
		serverGUI_Panel.setBackground(Color.darkGray);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == enter && message.length() > 0) {
			appendMessage("["+formatedNow+"]"+"서버 : "+message+"\n");
			sb.transmitAll("["+formatedNow+"]"+"서버 : "+message+"\n");
			chat.setText(null);
		}
	}
